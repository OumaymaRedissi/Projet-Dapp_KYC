...
contract Kyc {
  ...
  constructor()  {
      admin = msg.sender;
      //admin = 0x36b5A372761159F5637699b3eC0DfD7EeB9690d8;
  }
...
//Find the index of Org in KYC.organization array
  function findOrgIndex(address _custaddress,address ethAddress) internal view returns(uint a ){
      uint8 i = 0;
      for(i;i<customers[_custaddress].organization.length;i++){
            if(customers[_custaddress].organization[i]==ethAddress){
                return i;
  ...
  function findRequestIndex(address _custaddress,address _ethAddress) internal view returns(uint a){
      uint8 i = 0;
