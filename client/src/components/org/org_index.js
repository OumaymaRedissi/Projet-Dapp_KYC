...
const projectId = '2IaAkRTca0IGxGI9RlHmkEtk6ze';
const projectSecret = 'b81e46cb163b2d60c8f2039fae90194f';
const auth =
...
class Org extends Component {
  ...
  onRequest = (event) => {
    ...
    this.setState({ loading: true });
    this.state.kyc1
      .requestKYC(this.state.eth_address, { from: this.state.account })
      .then((req) => {
        this.setState({ loading: false });
        this.setState({ req });
        console.log(req);
  ...
  onDeleteRequest = (event) => {
    ...
    this.setState({ loading: true });
    this.state.kyc1
      .deleteRequestOrg(this.state.req_count, { from: this.state.account })
      .then((del) => {
        this.setState({
          loading: false,
        });
